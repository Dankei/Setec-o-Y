...
export class TweetController {
  ...
  async createTweet(request, response) {
    ...
    try {
      ...
      console.log("\n\n\ninfo: Finalizando TweetController.createTweet", request.result);  // Debug
  ...
  async findTweetById(request, response) {
    console.log("\n\n\ninfo: Iniciado TweetController.findTweetById", request.body); // Debug
    ...
    try {
      ...
      if (result) {
        ...
        console.log("\n\n\ninfo: Finalizando TweetController.findTweetById", request.result);  // Debug
  ...
  async findAllTweets(request, response) {
    console.log("\n\n\ninfo: Iniciado TweetController.findAllTweets", request.body); // Debug
    try {
      const result = await tweetService.findAllTweets();
      console.log("\n\n\ninfo: Finalizando TweetController.findAllTweets", request.result);  // Debug
      response.status(200).json(result);
    } catch (error) {
  ...
  async findTweetsByAuthorId(request, response) {
    console.log("\n\n\ninfo: Iniciado TweetController.findTweetsByAuthorId", request.body); // Debug
    const { authorId } = request.params;
    console.log(authorId);

    try {
      const result = await tweetService.findTweetsByAuthorId(authorId);
      console.log("\n\n\ninfo: Finalizando TweetController.findTweetsByAuthorId", request.result);  // Debug
      response.status(200).json(result);
    } catch (error) {
